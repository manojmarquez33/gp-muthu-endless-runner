# GP Muthu runner inspired by Google Dinosaur Game
...
pygame.display.set_caption("gp Run")
...
def draw_text(text, font_name, size, text_color, position_x, position_y, position):
    ...
    window.blit(text_plane, text_rect)  # blit() to draw


def load_image(path, size_x=0, size_y=0):

    image = pygame.image.load(path).convert_alpha()  # converts image into pixels
...
# class for creating and moving our gp buddy
class Gp:

    def __init__(self):

        self.idle_images = load_sprites("image/gp/", "idle_", 10, 200, 145)
        self.running_images = load_sprites("image/gp/", "run_", 8, 220, 153)
        self.jumping_images = load_sprites("image/gp/", "jump_", 16, 220, 153)
        ...
        # these indexes cycle through the images of the sprites, make the gp look moving
        self.idle_index = 0
    ...
    def check_collision(self, all_cactus):
        ...
        if self.running:
            gp_mask = pygame.mask.from_surface(self.running_images[self.running_index])
        elif self.jumping:
            gp_mask = pygame.mask.from_surface(self.jumping_images[self.jumping_index])
        else:
            gp_mask = pygame.mask.from_surface(self.idle_images[self.idle_index])
        ...
        collide = gp_mask.overlap(pygame.mask.from_surface(current_cactus[0]), offset_0) or \
            gp_mask.overlap(pygame.mask.from_surface(current_cactus[1]), offset_1)
...
def start_game():
    ...
    while run:
        ...
        if game_over:
            ...
            if gp.check_collision(cactus.get_cactus()):
                game_over = True
                game_over_screen.draw()
